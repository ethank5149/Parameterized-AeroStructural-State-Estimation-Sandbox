...
typed into :class:`~passes.flight.simulator.FlightConfiguration`: a length, a
...
:class:`~passes.aerodynamics.panels.PanelModel`, whose inputs are exactly a
...
does the vehicle glyph in :mod:`passes.viz.scene`. :meth:`to_body_axes`
...
class VehicleMesh:
    ...
    def to_body_axes(self, origin: str = "nose") -> VehicleMesh:
        ...
        :class:`~passes.aerodynamics.panels.PanelModel` and the vehicle
        glyph in :mod:`passes.viz.scene` both take the nose along +x; meshes
    ...
    def _ray_hits(
        ...
        from passes.batch.backend import get_array_module, to_numpy
