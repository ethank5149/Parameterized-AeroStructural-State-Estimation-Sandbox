...
:class:`~passes.flight.ballistic_entry.ExponentialAtmosphere` — one density,
    ...
    def state(self, altitude: ArrayLike) -> AtmosphereState:
        ...
        if np.any(z < self.floor) or np.any(z > self.ceiling):
            ...
            msg = (
                ...
                f"passes.atmosphere.EARTH, which stitches an upper model on."
            )
            raise ValueError(msg)
