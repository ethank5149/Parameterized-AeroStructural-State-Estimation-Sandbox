...
- **No published PICA decomposition kinetics exist in the reference set, so ours are pinned to stated targets instead (I-V4).** The MEDLI2 paper characterises conductivity, specific heat and density and is silent on Arrhenius triplets; the MSL reconstruction paper notes that *"no kinetic rate-limited recession model for PICA exists that is sufficiently validated for use in TPS design."* Rather than assert three numbers, the triplets are pinned to targets that are checked in the verification run — 2% mass loss by **557 K**, peak rate at **799 K** at 20 K/min, char yield **227/274** fixed by the published bulk densities rather than free. The useful part is the inverse: `passes.thermal.fiat.kinetics` forward-models a thermogravimetric scan and fits it, recovering the generating pre-exponentials and activation energies to **7×10⁻¹²** from a 3×-perturbed starting guess. One real TGA curve closes the largest remaining gap in the material model.
