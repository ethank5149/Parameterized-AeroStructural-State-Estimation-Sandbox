...
‖ II-V8 is **partially complete**. The implementation leg passes, and the **radiative half is now verified against the published source**: Tauber & Sutton's Table 1 and Eqs. (1)–(4) are transcribed from the PDF in [`reference/`](reference/), reproduced exactly at every tabulated node, and the blunting trade is demonstrated on published data rather than a surrogate. What remains is the *convective* half — the 5% criterion is stated against published Fay–Riddell reference conditions, which need transcribed equilibrium-air properties ($\rho_e\mu_e$, $\rho_w\mu_w$, $h_D$) the repository does not carry. The FIAT recession leg shares I-V4's pending status.
