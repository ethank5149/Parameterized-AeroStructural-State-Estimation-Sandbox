...
14. **FIAT ablation reference (I-V4).** ~~Needs the FIAT code~~ — **partly addressed.** FIAT is US-government-controlled, so `passes.thermal.fiat` now implements its published formulation independently (Chen & Milos 1999 Eqs. 1–11; Milos, Chen & Squire 2006), with multilayer stacks, B′ table closure, both in-depth radiation treatments, pressure-dependent conductivity from MEDLI2 Table 3, depth probes, environment scaling and ply-thickness optimization. [`docs/FIAT-fidelity.md`](docs/FIAT-fidelity.md) audits it feature-by-feature against what the papers describe and ranks what still drives disagreement — **kinetics first, B′ tables second, numerics last.** What remains is a **published reference case** carrying its own wall boundary condition and property set; [`docs/FIAT-reference-data.md`](docs/FIAT-reference-data.md) sets out what a usable one must contain and where to look.
